...
/** at-substitutes: (at) [at] {at} " at " AT-sign words. */
const AT_ALT = String.raw`(?:@|\(\s*at\s*\)|\[\s*at\s*\]|\{\s*at\s*\}|\s+at\s+|\s*_at_\s*|\s*-at-\s*)`;
/** dot-substitutes. */
const DOT_ALT = String.raw`(?:\.|\(\s*dot\s*\)|\[\s*dot\s*\]|\{\s*dot\s*\}|\s+dot\s+|\s*_dot_\s*|\s*-dot-\s*)`;

const OBFUSCATED_EMAIL_RE = new RegExp(
  String.raw`\b([a-z0-9._%+-]{2,64})\s*${AT_ALT}\s*([a-z0-9-]{2,64})\s*${DOT_ALT}\s*([a-z]{2,24})\b`,
  "gi",
);
